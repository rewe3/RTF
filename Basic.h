...
namespace Basic
{
    ...
    public:
        ...
        //David: direct implementation to utilize private members
        //compute entropy of every single point, by computing precision matrix of conditioned subgraph
        typename TValue*
        Entropy_for_each_point(const typename TTraits::DataSampler& traindb) const
        {
            ...
            for(size_t i = 0; i < traindb.GetImageCount(); ++i)
            {
                ...
                for(int y = 0; y < cy; ++y)
                {
                    for(int x = 0; x < cx; ++x)
                    {
                        ...
                        ConditionedSubgraph<TTraits>(Vector2D<int>(x, y), prep, ground, utypes, ptypes).ComputePrecisionMatrix(Theta_j);
                        entropies[x][y] = Entropy::ComputeEntropy(Theta_j);
