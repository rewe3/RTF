/* This file is part of the "Regression Fields" (RTF) source code distribution,
...
double UnarySmallestEigen    = 1e-2;
double UnaryLargestEigen     = 1e2;
double PairwiseSmallestEigen = 1e-2;
double PairwiseLargestEigen  = 1e2;
...
EvaluateAll(TModel& model, const std::string& systemName, long long trainTicks)
{
    ...
    //David: compute entropy of test picture
    rtf.Entropy_for_each_point(testds);
...
{
    const auto interimit      = 40;
    const auto finalit        = 100;
...
void PredictStacked()
{
    ...
    Denoising::Dataset testds(InputFile, Utility::ReadPNG<float, 3>(InputFile));

    auto prediction = rtf.Regress(testds, 1000, 1e-6, StackDepth-1);

    testds.SaveGroundTruthImagePNG(prediction[0], OutputFile);
}
...
void Usage(const std::string& name, int type)
{
    if( type == DISCRIMINATIVE )
        std::cerr << "Usage: ColorDenoising " << name << " <inputdir> <outputdir> <connectivity> <treedepth>" << std::endl;
    else if ( type == STACKED )
        std::cerr << "Usage: ColorDenoising " << name << " <inputdir> <outputdir> <connectivity> <treedepth> <stackdepth>" << std::endl;
    else
        assert(0); // invalid
...
{
    if( type == DISCRIMINATIVE )
        std::cerr << "Usage: ColorDenoising " << name << " <modelfile> <inputfile> <outputfile>" << std::endl;
    else if ( type == STACKED )
        std::cerr << "Usage: ColorDenoising " << name << " <modelfile> <inputfile> <outputfile> [<stackdepth>]" << std::endl;
    else
        assert(0); // invalid
...
std::string ParseArgumentsPredictDiscriminative(int argc, char* argv[], const std::string& name)
{
    ...
    std::string input = (argv[arg++]);
    SetInputFile(input, name, DISCRIMINATIVE);
...
int main(int argc, char* argv[])
{
    ...
    try
    {
        ...
        else if ( sys == "RTFTrainedForIWSSIM" )
        {
            auto name = ParseArgumentsDiscriminative(argc, argv, sys);
            Run([&]()
            {
                TrainAndEvaluate_DiscriminativeRTF<Loss::IWSSIM>(name);
            });
        }
        ...
        else if ( sys == "Compare" )
        {
            if( argc < 3 )
            {
                MyMonitor::Report("Usage: ColorDenoising Compare <loss>");
                ::exit(1);
            }
            auto loss = (argv[2]);
            if( loss == "MSE" )
            ...
            {
                MyMonitor::Report("Invalid <loss> argument (must be one of MSE, PSNR, MAE, SSIM, IWSSIM)\n");
                ::exit(1);
            }
        }
