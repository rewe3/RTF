...
namespace Compute
{
    ...
    public:
        ...
        //David: Compute Theta_j (Code from ConditionedSubgraph::ComputeMeanParameters())
        void ComputePrecisionMatrix(TMatrix &Theta_j) const
        {
            Theta_j = TMatrix::Zero();
            //maybe test using references directly in lambda abstraction
            TMatrix* pTheta_i = &Theta_j;
            for_each_conditioned_factor([ = ](const TConditionedFactor & factor)
                                        {
                                            *pTheta_i += factor.QuadraticCoefficients();
                                        });
